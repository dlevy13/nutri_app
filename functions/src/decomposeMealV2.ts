...
// --- Config (clé via .env ou via functions:config)
const MISTRAL_KEY =
  process.env.MISTRAL_API_KEY || functions.config().mistral?.key;
...
// --- Charge la table CIQUAL simplifiée (ton fichier JSON)
const FOOD_PATH = path.join(__dirname, "..", "assets", "food_data.json");
if (!fs.existsSync(FOOD_PATH)) {
  console.error("❌ assets/food_data.json manquant. Place le fichier dans functions/assets/food_data.json");
}
...
export const decomposeMeal = functions
  ...
  .https.onRequest(async (req, res): Promise<void> => {
    ...
    if (!CIQUAL.length) {
      res.status(500).json({ error: "Base CIQUAL vide (food_data.json non chargé)" });
      return;
    }
    ...
    try {
      ...
      // 1) Appel Mistral — prompt robuste + few-shots + température basse
      const payload = {
      ...
      // 2) Parsing LLM tolérant (array direct OU { ingredients: [...] })
      try {
        ...
        if (!items.length) {
          console.error("❌ LLM sans items utilisables. Contenu:", content.slice(0, 300));
          res.status(502).json({ error: "Réponse IA vide ou invalide" });
          return;
        }
      } catch (e) {
        console.error("❌ Parsing JSON LLM:", e, " / content:", content.slice(0, 300));
      ...
      for (const it of items) {
        ...
        // Fuzzy search avec normalisation + seuils
        ...
        }) as MatchData<FoodRow>[];

        const best = results[0];
        if (!best) continue;

        const matchedItem = best.item;
        const score = best.score; // 0..1
        if (score < 0.45) continue; // coupe-bas anti faux-positifs
      ...
      console.error("Erreur decomposeMeal:", err);
